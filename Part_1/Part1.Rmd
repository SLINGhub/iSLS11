...
runscatter <- function(data, var){
  plot_page <- function(data, nrows, ncols){
   ggplot(data, aes(x=RUN_ID, y={{var}})) + 
          ...
          stat_smooth(data= subset(data, QC_TYPE=="BQC"), aes(x=RUN_ID, y={{var}}), 
                      method = "loess", span = 0.75, na.rm = TRUE, se = FALSE, 
                      color = "brown3", linewidth = .7)+
          scale_shape_manual(na.value = NA, values = qc_shapes) +
          scale_fill_manual(values = qc_colors, na.value = NA) +
          scale_colour_manual(values = qc_colors, na.value = NA) +
          scale_x_continuous(breaks = seq(0, max(d_istd$RUN_ID), by = 100 )) + 
  ...
  # Save pages to a PDF. 
  pdf(file = here(paste0("Part_1/output/run_scatter_", quo_name(enquo(var)), "_all.pdf")),onefile = TRUE,
...
We observed some drifts in some lipid species - even after normalization with the ISTD. Such drifts an be corrected, which we will do. We assume that the `BQC` (=batch/process QC) represent drifts in our samples as well, and will apply the frequently used `loess` smoothing, based on the `BQC`. In the code below we log-transform the data during the smoothing to make it more robust. Smoothing parameter (i.e. `loess span`) depends on your data, we will chose the default of 0.7 in this case. Note, that for the analysis in the publication another algorithm, built-in to MRMkit , was used, that is based on the all data data rather than just the QCs.

```{r}
d_processed_corr <- d_processed

get_loess <- function(d) {
  tryCatch({
    dt <- tibble::tibble(RUN_ID = seq(min(d$RUN_ID), max(d$RUN_ID), 1))
    res <- stats::loess(
      data = subset(d, QC_TYPE == "BQC"),
      formula = CONC_LOG ~ RUN_ID,
      span = 0.75) %>%
...
## Calculate quality-control (QC) value for each lipid
...
d_qc_1 <- d_processed_corr |> 
  ...
  summarise(
    ...
    D_ratio = sd(CONC_ADJ[QC_TYPE == "BQC"])/sd(CONC_ADJ[QC_TYPE == "SAMPLE"])
  ) |> ungroup()
...
print(paste0("So, a total of ", nrow(d_qc[d_qc$QC_pass, ]), " of ", nrow(d_qc), " species passed QC..."))

```

So, a total of `r nrow(d_qc[d_qc$QC_pass, ])` of `r nrow(d_qc)` species passed QC...
